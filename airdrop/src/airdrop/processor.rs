...
use crate::airdrop::structs::{Address, Recipient, TokenAmount, Transactions};
...
impl AirdropProcessor {
    ...
    pub fn load_recipients_from_csv<P: AsRef<Path>>(
        ...
        let content = fs::read_to_string(path).map_err(|e| AirdropError::IOError(e.to_string()))?;
        ...
        for (i, line) in content.lines().enumerate() {
            ...
            let address_str = parts.next().ok_or(AirdropError::InvalidCSVFormat)?.trim();
            let amount_str = parts.next().ok_or(AirdropError::InvalidCSVFormat)?.trim();
    ...
    pub fn process_batch(
        ...
        for recipient in recipients {
            let transaction =
                Transactions::new(recipient.address.clone(), recipient.amount.clone());
        ...
        for transaction in transactions.iter_mut() {
            ...
            let result = self
                .blockchain
                .send_transaction(&transaction.recipient, &transaction.amount);
            match result {
                Ok(tx_id) => {
